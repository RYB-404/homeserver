...
def verify_password(password, stored_hash):
    ...
    # Backward compatibility for existing installs that stored plain SHA256.
...
def socketio_requires_permission(feature, level='any'):
    """Check SocketIO session permissions before allowing privileged events."""
    role = session.get('role', 'readonly')
    if not session.get('logged_in') or not has_permission(role, feature, level):
...
def _capability(name, available, detail, severity='info'):
    return {
        'name': name,
...
def get_health_status():
    ...
    capabilities = {
        'docker': _capability(
            ...
            docker_sock if os.path.exists(docker_sock) else 'Docker socket not mounted',
            'warning',
        ),
        'host_filesystem': _capability(
            'Host filesystem',
            os.path.exists(host_root) and host_root != '/',
            host_root if os.path.exists(host_root) else 'Host filesystem mount not available',
            'warning',
        ),
        'host_proc': _capability(
            'Host process namespace',
            os.path.exists(host_proc),
            host_proc if os.path.exists(host_proc) else 'Host /proc mount not available',
            'info',
        ),
        'host_sys': _capability(
            'Host hardware sensors',
            os.path.exists(host_sys),
            host_sys if os.path.exists(host_sys) else 'Host /sys mount not available',
            'info',
        ),
        'wireguard': _capability(
            'WireGuard config',
            os.path.exists(wireguard_dir),
            wireguard_dir if os.path.exists(wireguard_dir) else 'WireGuard config not mounted',
            'info',
        ),
        'temperature': _capability(
            'Temperature sensor',
            get_cpu_temperature() > 0,
            'Temperature data available' if get_cpu_temperature() > 0 else 'Temperature unavailable on this platform',
            'info',
        ),
    }

    host_control_keys = ['docker', 'host_filesystem', 'host_proc', 'host_sys']
    host_control_count = sum(1 for key in host_control_keys if capabilities[key]['available'])
    mode = 'host-control' if host_control_count >= 2 else 'safe'

    return {
        'mode': mode,
        'host_control_active': mode == 'host-control',
        'capabilities': capabilities,
        'summary': {
            'available': sum(1 for item in capabilities.values() if item['available']),
...
def get_storage_plan():
    base_paths = [
        {
            ...
            'purpose': 'Immich uploads, database, Redis, and ML cache',
        },
        {
            'id': 'media',
            'name': 'Media / Jellyfin',
            'path': '/mnt/media',
            'purpose': 'Movies, series, music, and subtitles',
        },
        {
            'id': 'websites',
            'name': 'Websites',
            'path': '/opt/homeserver/www',
            'purpose': 'Static or reverse-proxied website files',
        },
        {
            'id': 'apps',
            'name': 'App Config',
            'path': '/opt/homeserver',
            'purpose': 'Compose app configs and persistent service data',
        },
        {
            'id': 'backups',
            'name': 'Backups',
            'path': '/mnt/backups',
            'purpose': 'Dashboard config, compose files, and database dumps',
        },
    ]

    host_root = os.environ.get('HOST_ROOT', '/host/root')
    host_control = get_health_status()['host_control_active']
    for item in base_paths:
        host_path = item['path']
        check_path = os.path.join(host_root, host_path.lstrip('/')) if host_control else host_path
        item['exists'] = os.path.exists(check_path)
        try:
            usage_path = check_path if item['exists'] else os.path.dirname(check_path)
    ...
    return {
        'mode': 'host-control' if host_control else 'safe',
        'paths': base_paths,
        'notes': [
            'Use /mnt/photos for Immich uploads so photos can move to a larger disk later.',
            'Use /mnt/media for Jellyfin so media files stay separate from app configs.',
            'Keep /opt/homeserver for compose configs and small persistent app data.',
        ],
    }
...
def login_api():
    ...
    password = data.get('password', '')
    # Check main user
    if username == config['username'] and verify_password(password, config['password_hash']):
        if not config['password_hash'].startswith('pbkdf2_sha256$'):
            config['password_hash'] = hash_password(password)
            save_security_config(config)
        session['logged_in'] = True
        session['username'] = username
...
@app.route('/api/storage/plan')
@login_required
def storage_plan_api():
    return jsonify(get_storage_plan())
...
def handle_start_terminal(data):
    if not socketio_requires_permission('terminal'):
        return
...
def docker_containers():
    ...
    if not health['capabilities']['docker']['available']:
        return jsonify([])
...
@requires_permission('security_policy')
...
def get_installed_apps():
    ...
    if not get_health_status()['capabilities']['docker']['available']:
        return jsonify({'installed': [], 'available': False})
...
SYSTEM_APPS = [
    {"id": "health", "name": "Health", "icon": "fa-solid fa-heart-pulse", "color": "linear-gradient(135deg, #0f9b8e, #78ffd6)", "url": "/health"},
